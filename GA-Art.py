...
# Define constants for genetic algorithm
POPULATION = 100
INITIAL_GENES = 200
GENERATIONS = 60000
MAX_RADIUS = 30
TARGET_ARR = np.asarray(Image.open("Goose.jpg"))
...
def crossOver(population):
    ...
    while(i != POPULATION//2):
        ...
        child.extend(g.copy() for g in parent2[len(parent2)//2:])
...
def main():
    ...
    for i in range(GENERATIONS):
        ...
        if ((i+1)%100 == 0):
            ...
            result.save(f"Output/Generations/gen-{i+1}.png")
            print(f"The total gene count of best is {len(pop[len(pop)-1][1])}")
